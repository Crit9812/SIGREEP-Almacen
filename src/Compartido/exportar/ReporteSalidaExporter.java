...
public class ReporteSalidaExporter extends AbstractPDFExporter<traspasoSalida, ReporteSalidaExporter.DetalleSalidaPDF> {

    private final TipoOperacion tipoOperacion;

    // Para mantener compatibilidad
    public ReporteSalidaExporter() {
        this(TipoOperacion.TRASPASO_SALIDA);
    }

    public ReporteSalidaExporter(TipoOperacion tipoOperacion) {
    ...
    // Clase interna para detalles de salida (igual que antes)
    ...
    @Override
    protected List<DetalleSalidaPDF> convertirItemsADetallesPDF(List<traspasoSalida> items) {
    ...
    @Override
    protected Map<String, List<UbicacionCompra>> extraerUbicacionesPorProducto(List<traspasoSalida> items) {
    ...
    protected void dibujarFilaDetalle(PDPageContentStream contentStream,
                                      ...
                                      DetalleSalidaPDF detalle, int filaNum, float xPos) throws IOException {
        ...
        if (tipoOperacion == TipoOperacion.TRASPASO_SALIDA) {
            // Columna 6: Presentación (Traspaso Salida)
            contentStream.beginText();
            contentStream.setFont(fontDatos, 9);
            contentStream.newLineAtOffset(xPos + PDFCommons.CELL_PADDING, currentY - 15);
            ...
            // Columna 7: Factor (Traspaso Salida)
            contentStream.beginText();
            contentStream.setFont(fontDatos, 9);
            contentStream.newLineAtOffset(xPos + PDFCommons.CELL_PADDING, currentY - 15);
    ...
    protected float dibujarInformacionGeneral(PDPageContentStream contentStream,
                                              ...
                                              List<DetalleSalidaPDF> detalles, String nombreDestinatario,
        ...
        if (tipoOperacion != TipoOperacion.VENTA) {
            String textoOrigen = "Remitente: ";
            String valorOrigen = "Almacén";
        ...
        String textoDestino = tipoOperacion == TipoOperacion.VENTA ? "Cliente: " : "Destino: ";
        String valorDestino = nombreDestinatario != null ? nombreDestinatario : "No especificado";

        contentStream.beginText();
        contentStream.setFont(fontNormal, 11);
        contentStream.newLineAtOffset(col1X, currentY - (lineCount * PDFCommons.LINE_HEIGHT));
        contentStream.showText(textoDestino + valorDestino);
        contentStream.endText();
    ...
    @Override
    protected String obtenerClaveProducto(DetalleSalidaPDF detalle) {
        return detalle.getClaveProducto();
    }

    @Override
    protected String obtenerNombreProducto(DetalleSalidaPDF detalle) {
        return detalle.getProducto();
    }

    // Métodos estáticos para mantener compatibilidad
    public static void exportarReporteTraspasoSalida(String claveSalida,
                                                     String nombreSucursalDestino,
                                                     String comentario,
                                                     List<traspasoSalida> itemsTraspaso,
                                                     Window owner) {
        new ReporteSalidaExporter(TipoOperacion.TRASPASO_SALIDA)
                .exportar(claveSalida, nombreSucursalDestino, comentario, itemsTraspaso, owner);
    }

    public static void exportarReporteVenta(String claveVenta,
                                            String nombreCliente,
                                            String comentario,
                                            List<traspasoSalida> itemsVenta,
                                            Window owner) {
        new ReporteSalidaExporter(TipoOperacion.VENTA)
                .exportar(claveVenta, nombreCliente, comentario, itemsVenta, owner);
    }
}